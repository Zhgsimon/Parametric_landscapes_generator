...
Soleil::Soleil(std::mt19937& rg)
    : Astre{{util::alea(0, 1366, rg), util::alea(50, 250, rg)}}, m_soleil{m_coords, util::alea(60, 110, rg),  {255, 215, 0}}
{}
...
///Apres avoir dessiner le soleil dans le svg.file on rajoute des effets de brillance et de gradation
    for (int i=1; i<10; ++i)
        svgout.addDisk(m_coords.getX(), m_coords.getY(), m_soleil.getRayon()+i*1.5, makeRGB(255,215+10*i,i*10), 0.1+(i/10));//{255, 115+(i/2), 0}});

    for (int i=1; i<10; ++i)
        svgout.addDisk(m_coords.getX(), m_coords.getY(), m_soleil.getRayon()+7-i, makeRGB(255,215,0), 0.1+(i/10));//{255, 115+(i/2), 0}});
}
